...
def LRA_loss(inputs, targets, criterion, epoch, lambda_, model, norm, epoch_a, epoch_b):
    ...
    epoch_coef = 0.0 if epoch < epoch_a else ((epoch - epoch_a) / (epoch_b - epoch_a)) if epoch < epoch_b else 1.0
...
for epoch in range(epochs):
    ...
    for train_input, train_target in trainloader:
        ...
        loss = LRA_loss(out, train_target, criterion, epoch, lambda_, model, 'nuc', epoch_a, epoch_b)
        loss.backward()
    ...
    if regressionTask:
        print("Epoch %i : train loss %0.3f, val loss %0.3f, trace norm %0.1f"%(epoch, train_loss, val_loss, trace_norm))
...
if regressionTask:
    print("Best train loss: %0.3f, best val loss: %0.3f (at epoch %i)"%(np.min(train_losses), np.min(val_losses), np.argmin(val_losses)))
